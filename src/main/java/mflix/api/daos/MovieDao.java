...
public class MovieDao extends AbstractMFlixDao {
  ...
  public List<Document> getMoviesByCountry(String... country) {
    ...
    // As a user, I'd like to be able to search movies by country and see a list of movie titles.
    // I should be able to specify a comma-separated list of countries to search multiple countries.

    List<Document> movies = new ArrayList<>();
    Bson queryFilter = in("countries", Arrays.asList(country));
    Bson titleProjection = new Document("title",1);
  ...
  private Bson buildFacetStage() {
    ...
    // Arrays.asList(unwind("$cast"), match(eq("cast", "Paul Newman")))
  }
